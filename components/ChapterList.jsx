...
export default function ChapterList({
  ...
  return (
    <div
      id="all-chapters"
      className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-600 overflow-hidden"
    >
      ...
      {totalPages > 1 && (
        <div className="p-4 border-t border-gray-200 dark:border-slate-600 flex justify-center items-center gap-4 bg-gray-50 dark:bg-slate-900">
          ...
          <input
            type="number"
            // Shows "1" (or current page) as a greyed out hint, but input stays empty for typing
            value={pageInput}
            placeholder={currentPage.toString()}
            onChange={(e) => {
              const value = e.target.value;
              // Prevent typing numbers higher than totalPages
            ...
            min={1}
            max={totalPages}
            // Removes the default "spin buttons" (up/down arrows) for a cleaner look
            className="w-16 px-2 py-1 rounded border border-gray-300 dark:border-slate-600 text-center text-sm bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
          />
          <span className="text-sm text-gray-600 dark:text-slate-400">
            of {totalPages}
          </span>
