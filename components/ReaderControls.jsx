    ...
    <>
      ...
      <div className="hidden sm:block p-3 bg-gray-50 dark:bg-slate-800 rounded-md shadow-sm">
        <div className="flex gap-3 items-center">
          ...
          <label className="flex items-center gap-2">
            ...
            <button onClick={() => { setReaderTheme('day'); setBgColor('#ffffff'); setTextColor('#111827') }} className="px-2 py-1 rounded border text-sm">Day</button>
            <button onClick={() => { setReaderTheme('night'); setBgColor('#0f172a'); setTextColor('#e5e7eb') }} className="px-2 py-1 rounded border text-sm">Night</button>
            <button onClick={() => { setReaderTheme('sepia'); setBgColor('#f5ecd8'); setTextColor('#111827') }} className="px-2 py-1 rounded border text-sm">Sepia</button>
          </label>

          <div className="ml-auto">
            <button onClick={() => { setFontSize(18); setFontFamily(FONT_FAMILIES[0].css); setTextColor('#111827'); setBgColor('#ffffff'); setReaderTheme('day') }} className="px-3 py-1 rounded border">Reset</button>
          </div>
        </div>
      </div>
      ...
      <div className="sm:hidden fixed bottom-3 left-1/2 transform -translate-x-1/2 z-50 w-[92%]">
        <div className="reader-controls-mobile bg-white dark:bg-slate-900 rounded-xl p-2 shadow-lg flex items-center justify-between">
          ...
          <button aria-label="Reader theme toggle" onClick={() => setReaderTheme(readerTheme === 'night' ? 'day' : 'night')} className="px-3 py-2 rounded">
            {readerTheme === 'night' ? '🌙' : '☀️'}
          </button>
          ...
          <button onClick={() => { setFontSize(18); setFontFamily(FONT_FAMILIES[0].css); setTextColor('#111827'); setBgColor('#ffffff'); setReaderTheme('day') }} className="px-3 py-2 rounded">Reset</button>
        </div>
      </div>
