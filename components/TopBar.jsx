...
export default function TopBar({ onOpenChapters, onSearch, initialSearch='' }) {
  const [q, setQ] = useState(initialSearch)
  const [siteTheme, setSiteTheme] = useState('light') // 'light' or 'dark'

  useEffect(() => {
    try {
      const t = typeof window !== 'undefined' && window.localStorage.getItem('siteTheme')
    ...
    <header className="bg-white dark:bg-slate-900 border-b dark:border-slate-700 sticky top-0 z-40">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 h-14">
          <button aria-label="Open chapters" onClick={onOpenChapters} className="p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 sm:hidden">
          ...
          <form onSubmit={handleSubmit} className="flex-1">
            <div className="relative">
              <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search chapters or text..." className="w-full rounded-md border px-3 py-2 pl-10 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="Search chapters" />
          ...
          <div className="flex items-center gap-2">
            <button onClick={toggleSiteTheme} className="p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="Toggle site theme">
              {siteTheme === 'dark' ? (
